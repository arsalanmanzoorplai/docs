...
<ParamField body="thankYouPage" type="object" required>
	<Expandable title="Thank You Page Object">
		...
		<ParamField body="button_text" type="string" required>
			...
			<Note>
				This field must be **required** when `button_type` is set to `VISIT_WEBSITE`. For all other button types, this field is **optional**.
			</Note>
		</ParamField>
				<ParamField body="country_code" type="string">
			ISO 3166-1 alpha-2 country codes e.g: `US`,`UK`
				<Note>
				This field must be **required** when `button_type` is set to `CALL_BUSINESS`. For all other button types, this field is **optional**.
			</Note>
		</ParamField>
